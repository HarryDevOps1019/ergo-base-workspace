  ...
  head: () => ({
    meta: [
      { title: "Ergonomic Chairs — Gaming & Office | Ergo Base" },
      {
        name: "description",
        content:
          "Premium ergonomic gaming chairs and office workstation chairs in Sri Lanka. 4D armrests, lumbar support, breathable mesh — built for long hours.",
      },
      {
        property: "og:title",
        content: "Ergonomic Chairs — Gaming & Office | Ergo Base",
      },
      {
        property: "og:description",
        content:
          "Premium ergonomic gaming chairs and office workstation chairs in Sri Lanka.",
      },
    ],
  }),
...
function ChairsPage() {
  ...
  const filtered =
    activeCategory === "all"
      ? chairs
      : chairs.filter((c) => c.category === activeCategory);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative h-[60vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        <img
          src={chairsHero}
          alt="Ergo Base chair collection"
        ...
        <div className="relative z-10 text-center max-w-3xl mx-auto px-4">
          <p className="text-gold text-sm font-semibold tracking-[0.3em] mb-4">
            ERGO BASE SEATING
          </p>
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-4">
            Sit Better. Play Better. Work Better.
          </h1>
          <p className="text-muted-foreground text-lg max-w-xl mx-auto">
            Premium ergonomic chairs engineered for marathon gaming sessions and
            all-day office productivity.
          </p>
        </div>
      </section>
      ...
      <div className="border-y border-border bg-surface">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-wrap items-center justify-center gap-8 text-sm text-muted-foreground">
          <span className="flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-gold" /> 2 Year Warranty
          </span>
          <span className="flex items-center gap-2">
            <Truck className="w-4 h-4 text-gold" /> Free Delivery in Colombo
          </span>
          <span className="flex items-center gap-2">
            <Star className="w-4 h-4 text-gold" /> 4.8★ Average Rating
          </span>
        </div>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filtered.map((chair) => (
            <div
              key={chair.id}
              className="group bg-surface border border-border rounded-2xl overflow-hidden hover:border-gold/30 transition-all duration-300"
            >
              <div className="relative aspect-square overflow-hidden bg-background">
                ...
                {chair.originalPrice && (
                  <span className="absolute top-4 right-4 bg-destructive text-destructive-foreground text-xs font-bold px-3 py-1 rounded-full">
                    SAVE{" "}
                    {Math.round(
                      ((chair.originalPrice - chair.price) /
                        chair.originalPrice) *
                        100,
                    )}
                    %
              ...
              <div className="p-6">
                <p className="text-xs text-gold tracking-widest mb-1 uppercase">
                  {chair.category === "gaming"
                    ? "Gaming"
                    : "Office / Workstation"}
                </p>
                ...
                <div className="flex items-center gap-1 mb-3">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      key={i}
                      className={`w-3.5 h-3.5 ${i < Math.floor(chair.rating) ? "text-gold fill-gold" : "text-muted-foreground"}`}
                    />
                  ))}
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {chair.features.map((f) => (
                    <span
                      key={f}
                      className="text-[10px] bg-accent text-accent-foreground px-2 py-1 rounded-full"
                    >
                      {f}
                    </span>
                  ))}
                ...
                <div className="flex items-end gap-2 mb-4">
                  <span className="text-2xl font-bold text-gold">
                    LKR {chair.price.toLocaleString()}
                  </span>
                  {chair.originalPrice && (
                    <span className="text-sm text-muted-foreground line-through">
                      LKR {chair.originalPrice.toLocaleString()}
                    </span>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  <Button
                    variant="gold"
                    className="flex-1"
                    onClick={() => navigate({ to: `/product/${chair.id}` })}
                  >
                    Buy Now
                  </Button>
                  <Button
      ...
      <section className="bg-surface border-y border-border">
        <div className="max-w-7xl mx-auto px-4 py-20">
          ...
          <p className="text-muted-foreground text-center max-w-2xl mx-auto mb-12">
            Every chair is hand-selected and tested by our ergonomics team to
            meet Sri Lanka's demanding climate and workstyle.
          </p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              {
                title: "Engineered Lumbar",
                desc: "Adjustable lumbar systems that adapt to your spine's natural curve.",
              },
              {
                title: "Premium Materials",
                desc: "Cold-cure foam, breathable mesh, and PU leather built for tropical heat.",
              },
              {
                title: "Heavy-Duty Bases",
                desc: "Class 4 gas lifts and steel/aluminium bases rated for 150kg+.",
              },
              {
                title: "2-Year Warranty",
                desc: "Full coverage on frame, mechanism, and upholstery — no questions.",
              },
            ].map((item) => (
      ...
      <section className="max-w-7xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-muted-foreground mb-8 max-w-xl mx-auto">
          Book a free ergonomic consultation and our experts will help you pick
          the perfect chair for your body and setup.
        </p>
